...
machines. The work is I/O-bound (Python releases the GIL around blocking syscalls), so a small
``QThreadPool`` overlaps the Box latency and keeps the UI responsive.
...
# I/O-bound (waiting on Box stat latency), not CPU-bound: pick a fixed width independent of core
# count so a low-core laptop still runs several Box waits at once, without hammering the client.
_MAX_CONCURRENCY = 8
...
class SubjectSummaryScanner(QObject):
    """Owns a thread pool; fans subject-status computation out and re-emits results on the GUI
    thread. Create it on (and parent it to) a main-thread widget."""
