...
- **Loading / re-scanning the subject list froze the app on low-CPU machines (e.g. a Mac laptop
  over Box).** Each subject's status was computed on the UI thread, and every
  ...
  many seconds. It now runs on a **single background worker**, and results stream back per subject
  via a queued signal — the UI stays responsive throughout. The scan is deliberately **serial, not
  parallel**: Box's virtual filesystem serializes concurrent placeholder access and locks up under
  load, so issuing many `stat()`s at once saturated the Box driver and froze the whole machine (the
  OS file explorer included), not just the app. Moving the work off the UI thread — not scanning
  faster — is the fix. The directory listing (`scan_subjects`) also moved off the UI thread. A
  generation token drops results from a superseded scan (task switch / re-scan), and the worker is
  torn down on app close. (New `gui/workers/summary_scanner.py` `SubjectSummaryScanner`;
  `Dashboard._scan` / `_on_subjects_listed` / `_on_summary_ready`; `MainWindow.closeEvent`.)
