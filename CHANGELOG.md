...
- **Double-click launcher.** `scripts/make_launcher.ps1` drops a Desktop shortcut that opens the GUI
  with no console window (via the `rpcoding-gui` gui-script) and the brain icon. Command-line launch
  is unchanged (`rpcoding-gui` or `python -m rpcoding.gui.app`).
