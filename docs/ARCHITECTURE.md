...
- **Playback** ([editor/playback.py](../src/rpcoding/gui/editor/playback.py)): Space / ▶ Play streams
  ...
  `OutputStream` callback — no RAM blow-up for long ranges — applying the **Vol** gain per block. The
  **cursor doubles as the playhead**: a UI-thread `QTimer` advances it to `player.position()` and
  pages the view to follow; on stop/finish the cursor is left at that position. `finished` is emitted
  from the audio thread and delivered queued on the UI thread to reset the button; playback stops on
  close. All sounddevice use is guarded, so a missing audio backend is a silent no-op.
